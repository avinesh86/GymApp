...
def test_provision_creates_user_membership_and_link(tenant, admin_user, mailoutbox):
    staff = StaffProfileFactory(tenant=tenant, name="Sarah Mitchell", email=_email(), role="instructor")

    user, created = provision_user_for_staff(staff, send_invite=True)
...
def test_provision_links_existing_same_tenant_user(tenant):
    email = _email()
    existing = UserFactory(tenant=tenant, email=email, role="gym_manager")
    staff = StaffProfileFactory(tenant=tenant, email=email, role="instructor")

    user, created = provision_user_for_staff(staff)

    assert created is False
    assert user.id == existing.id
    assert staff.user_id == existing.id
...
def test_provision_reuses_global_user_across_gyms(tenant, other_tenant, mailoutbox):
    ...
    email = _email()
    staff_a = StaffProfileFactory(tenant=tenant, email=email, name="Jo Multi", role="instructor")
    staff_b = StaffProfileFactory(tenant=other_tenant, email=email, name="Jo Multi", role="gym_manager")

    user_a, created_a = provision_user_for_staff(staff_a, send_invite=True)
    user_b, created_b = provision_user_for_staff(staff_b, send_invite=True)
...
def test_set_password_with_valid_token(tenant):
    staff = StaffProfileFactory(tenant=tenant, email=_email(), role="instructor")
    user, _ = provision_user_for_staff(staff)
...
def test_set_password_rejects_bad_token(tenant):
    staff = StaffProfileFactory(tenant=tenant, email=_email())
    user, _ = provision_user_for_staff(staff)
...
def test_validate_invite_endpoint(tenant):
    staff = StaffProfileFactory(tenant=tenant, email=_email(), name="Val Idate")
    user, _ = provision_user_for_staff(staff)
    uid, tok = make_invite_token(user)
